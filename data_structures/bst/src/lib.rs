                    ...
                    match bnode.right {
                        ...
                        Some (ref mut rnode) => {
                            match prev {
                                None => return (),
                                Some(rawt) => {
                                    BST::remove_node_int(rnode.as_raw(), rawt, elem);
                                }
                            }
                        }
                    }
                ...
                cur_node.as_mut().map(
                    |node| {
                        if node.left.is_none() && node.right.is_none() {
                            ...
                            prev_node.as_mut().map(|n| {
                                if is_left {
                                    n.left = node.right.take();
                                } else {
                                    n.right = node.right.take();
                                }
                            });
